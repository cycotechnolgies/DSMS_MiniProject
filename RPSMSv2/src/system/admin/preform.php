...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
              ...
              $sql = "INSERT INTO preforme (schId, stuId, preform, vehi, grade, hrs, reason) 
              VALUES (:schId, :stuId, :preform, :vehi, :grade, :hrs, :reason)";
              $stmt = $conn->prepare($sql);

              // Bind parameters
              $stmt->bindParam(':schId', $cid, PDO::PARAM_INT);
              $stmt->bindParam(':stuId', $sid, PDO::PARAM_INT); 
              $stmt->bindParam(':preform', $req, PDO::PARAM_STR);
              $stmt->bindParam(':vehi', $vehi, PDO::PARAM_STR);
              $stmt->bindParam(':grade', $grade, PDO::PARAM_STR);
              $stmt->bindParam(':hrs', $hrs, PDO::PARAM_INT);
              $stmt->bindParam(':reason', $res, PDO::PARAM_STR);
  
              // Execute the prepared statement
              if ($stmt->execute()) {
                    echo "New record created successfully";
                    header('Location: cls_detail.php?sid='.$cid.'&cGFpZCBhdA');
                    exit();
                }else {
      ...
      <div class="drawer-content flex flex-col">
        ...
        <!-- profile details -->
            ...
            <div class="flex flex-col items-center w-full">
                <?php
                    if($errors){
                        echo '<div class="p-4 bg-red-200 text-red-500 uppercase">
                        <p>'.$errors.'</p>
                    </div>';
                    }
                ?>
