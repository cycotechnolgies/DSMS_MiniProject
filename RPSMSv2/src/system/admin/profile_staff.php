...
<?php
        ...
        $query1 = "SELECT * FROM staff WHERE userId = :userId LIMIT 1;";
        $rows1 = $conn->prepare($query1);
        $rows1->bindParam(':userId', $userId);
        $rows1->execute();
        $result1 = $rows1->fetch(PDO::FETCH_ASSOC);
        ...
        if ($rows1->execute()) {
            if ($result1) {
                ...
                $addr = $result1['addr'];
                $nic = $result1['nic'];
                $passNo = $result1['passNo'];
                $tel = $result1['tel'];
                $waNo = $result1['waNo'];
            }
        } else {
          $errorInfo = $rows1->errorInfo();
                        ...
                        <div class="bg-white shadow rounded-lg p-6">
                            <div class="flex flex-col items-center">
                                ...
                                <div class="mt-6 flex flex-wrap gap-4 justify-center">
                                    ...
                                    <button  class="px-4 py-2 rounded-md bg-green-600 text-white " id="uptBtn"  type="button" onclick="window.location.href='staff_attend.php?std=<?php echo $userId; ?>'">Attendent</button>
                                </div>
                                <div class="mt-6 flex flex-wrap gap-4 justify-center">
                    ...
                    <div class="col-span-4 sm:col-span-9">
                        <div class="bg-white shadow rounded-lg p-2">
                            ...
                            <form action="" method="post" id="form1">
                                <div>
                                    ...
                                    <div>
                                      ...
                                      <input class="border my-2 border-gray-400 rounded-md w-full p-2 focus:border-blue-500 focus:outline-none" type="text" id="nic" name="nic" maxlength="12" value="<?php echo $nic; ?>"   placeholder="96******* V / 20**********" required />
                                    </div>
                                      <div>
                                      <label for="ppn">Passport No. / විදේශ ගමන් බලපත්‍ර අංකය</label><br>
                                      <input class="border my-2 border-gray-400 rounded-md w-full p-2 focus:border-blue-500 focus:outline-none" type="text" id="ppn" name="ppn" placeholder="N17******" value="<?php echo $passNo; ?>"   maxlength="10" />
                                    </div>
                                    </div>
                                    <div>
                                      <label for="addr">Permenent Address / ස්ථීර ලිපිනය<span class="text-red-500">*</span></label>
                                      <input class="border my-2 border-gray-400 rounded-md w-full p-2 focus:border-blue-500 focus:outline-none" type="text" id="addr" name="addr" value="<?php echo $addr; ?>"   placeholder="No. 152, ***********,********" required />
                                    </div>
                                    <div class="cols grid md:grid-cols-2 gap-4">
                                      <div>
                                        <label for="mobi">Mobile No. / දුරකථන අංකය<span class="text-red-500">*</span></label>
                                        <input class="border my-2 border-gray-400 rounded-md w-full p-2 focus:border-blue-500 focus:outline-none" type="text" id="mobi" name="tel" maxlength="10" value="<?php echo $tel; ?>"   placeholder="07* *******" required />
                                      </div>
                                        <div>
                                          <label for="wa">WhatsApp No. / වට්ස්ඇප් අංකය<span class="text-red-500">*</span></label>
                                          <input class="border my-2 border-gray-400 rounded-md w-full p-2 focus:border-blue-500 focus:outline-none" type="text" id="wa" name="wa" placeholder="07* *******" value="<?php echo $waNo; ?>"   maxlength="10"required />
                                      </div>
                                    </div>
                            ...
                            <div class="flex gap-4 justify-end hidden" id="formBtn">
                                ...
                                <button onclick="" class="px-4 py-2 rounded-md bg-green-600 text-white " id="editupdate" data-student-id="<?php echo $userId;?>"  type="button"><i class="fa-solid fa-angles-up"></i> Update</button>
