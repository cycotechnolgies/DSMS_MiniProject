        ...
        <div class="p-4 h-screen">
          ...
          <?php
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $query = "SELECT * FROM medi_payments WHERE mpay_id LIKE :find OR mediId LIKE :find OR  userId LIKE :find OR payType LIKE :find OR mpay_id LIKE :find OR amount LIKE :find OR reason LIKE :find OR pay_state LIKE :find AND userId = :uids;";
                ...
                $rows->bindParam(':uids', $_SESSION['uids']);
                ...
                $query = "SELECT *, DATE(pay_date) AS date FROM medi_payments WHERE userId = :uids ORDER BY mpay_id DESC;";
                $rows = $conn->prepare($query);
            ...
            if ($result) {
                foreach ($result as $row) {
                    ...
                    echo '
                        <div class="bg-blue-200 rounded-xl shadow-md overflow-hidden" onclick="window.location.href=\'mpay_details.php?uid='.$userId.'&bm90bnVsbA&pid='.$payId.'&cGFpZCBhdA\'">
