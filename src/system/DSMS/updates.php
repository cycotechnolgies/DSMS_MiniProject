...
if (isset($_POST['uptSub'])) {
    ...
    $query = "INSERT INTO progress(userId, events, des, eventDate, userState) VALUES (:userid, :events, :des, :dates, :ustatus);";
    
    $stmt = $conn->prepare($query);
  
    $stmt->bindParam(':userid', $userId);
    $stmt->bindParam(':events', $event);
    ...
    header("Location: updates.php?uid=$userId");
    exit();
  }
  ...
  if(isset($_POST['del'])){
    ...
    $sql = "DELETE FROM progress WHERE proId = :dels";
    $delstmt = $conn->prepare($sql);

    $delstmt->bindParam(':dels', $delId);
            ...
            <div class="relative overflow-hidden p-4">
                <table class="table-auto w-full text-left">
                    ...
                    <tbody class="bg-white text-gray-500">
                        <?php
                        $sql = "SELECT * FROM progress WHERE userId = :uids;";
                        $stmt = $conn->prepare($sql);
                        $stmt->bindParam(':uids', $userId, PDO::PARAM_INT);
                        $stmt->execute();
                        ...
                        if ($results) {
                            foreach ($results as $result) {
                                echo '<tr class="py-2">
                                    <td class="py-2 border text-center  p-4">' . $result['eventDate'] . '</td>
                                    <td class="py-2 border text-center  p-4">' . $result['events'] . '</td>
                                    <td class="py-2 border text-center  p-4">' . $result['des'] . '</td>
