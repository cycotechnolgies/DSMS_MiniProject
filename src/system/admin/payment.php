        ...
        <div class="p-4 h-screen">
          ...
          <?php
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $query = "SELECT *, DATE(date) AS date FROM payments WHERE payId LIKE :find OR  userId LIKE :find OR payType LIKE :find OR branch LIKE :find OR branch LIKE :find OR amount LIKE :find OR reason LIKE :find OR pay_state LIKE :find;";
                ...
                $query = "SELECT *, DATE(date) AS date FROM payments ORDER BY payId DESC;";
                $rows = $conn->prepare($query);
                $rows->execute();
                $result = $rows->fetchAll(PDO::FETCH_ASSOC);
