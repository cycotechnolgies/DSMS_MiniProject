...
$cls_query = "SELECT dateTime
FROM schedule
WHERE schedId = :cid;";

$rows = $conn->prepare($cls_query);
// $rows->bindParam(':userId', $userId);
$rows->bindParam(':cid', $sId);

if($rows->execute()){
  
  $result = $rows->fetch(PDO::FETCH_ASSOC);

  $Times = $result['dateTime'];
}

//stu detail code
$query = "SELECT s.*, r.*
FROM student s
JOIN requests r ON s.userId = r.userId
WHERE s.userId = :uids;";

$rows = $conn->prepare($query);
// $rows->bindParam(':userId', $userId);
$rows->bindParam(':uids', $userId);
...
if (isset($_POST['submit'])) {
  ...
  // Correct SQL query with proper placeholders and VALUES keyword
  $query_card = "INSERT INTO cards(userId, ClassId, Types, Exp_Date, Hrs) VALUES (:uids, :cids, :types, :exp, :hrs)";

  $stmt = $conn->prepare($query_card);

  // Binding parameters with the correct placeholders
  $stmt->bindParam(':uids', $userId);
  $stmt->bindParam(':cids', $sId);
  $stmt->bindParam(':types', $types);
  $stmt->bindParam(':exp', $expDate);
  $stmt->bindParam(':hrs', $hrs);

  $stmt->execute();

  header('Location: add_clsStu.php?cid=' . $sId);
  exit(); // it's a good practice to use exit() after a header redirection
}
