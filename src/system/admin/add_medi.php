...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $inst = $_POST['inst'];
    $branch = $_POST['branch'];
    $medifor = $_POST['medifor'];
    $appoint = "pending";
    $refId = "pending";


    // Prepare an SQL statement to insert data into the database
    $query = "INSERT INTO medic(userId, fullName, gen, nic, passNo, addr, tel, waNo, apponite, refId, branch, medfor, inst) 
    VALUES (:userId, :fullName, :gen, :nic, :passNo, :addr, :tel, :waNo, :apponite, :refId, :branch, :medifor, :inst);";
    ...
    $stmt->bindParam(':apponite', $appoint);
    $stmt->bindParam(':refId', $refId);
    $stmt->bindParam(':branch', $branch);
    $stmt->bindParam(':medifor', $medifor);
    ...
    try {
        ...
        header("Location: add_paysel_medi.php?mid=".$medId);
        exit();
