...
if (isset($_POST['uptPay'])) {
    ...
    $sql = "UPDATE medi_payments SET pay_state = :newPayState WHERE mpay_id = :payId";
        ...
        if ($payUpdate == 'Paid') {
            if (file_exists($slip)) { // Check if the file exists
                if (unlink($slip)) { // Attempt to delete the file
                    ...
                    // Update the database
                    $query = "UPDATE medi_payments SET slip = :slip WHERE mpay_id = :payId";
                    $stmt = $conn->prepare($query);
                    $stmt->bindParam(':slip', $fileremove, PDO::PARAM_STR);
                    $stmt->bindParam(':payId', $payId, PDO::PARAM_INT);
                    $stmt->execute(); // Execute the statement
        
                    header("Location: mpay_details.php?uid=$userId&bm90bnVsbA&pid=$payId&cGFpZCBhdAFDS");
            ...
            header("Location: mpay_details.php?uid=$userId&bm90bnVsbA&pid=$payId&cGFpZCBhdA");
            exit(); // Stop execution after redirection
        }
...
if(isset($_POST['delbtn'])){
  ...
  $query = "DELETE FROM medi_payments WHERE mpay_id = :payId;";
  $stmt = $conn->prepare($query);
        ...
        <div class="p-4">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="grid grid-cols-2 md:grid-cols-2 gap-4">
                <a href="../phpconfigs/prints_medi.php?pid=<?php echo $payId;?>&uid=<?php echo $mId;?>" target="_blank">
                  <button class="px-4 py-2 rounded-md bg-blue-600 text-white w-full">Print</button>
                </a>
              <div>
                <form action="" method="post">
                  <input type="text" name="payid" value="<?php echo $payId; ?>" hidden> 
          ...
          <div class="relative overflow-hidden">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <input type="text" name="" id="Uid" value="<?php echo $mId; ?>" class="w-full py-2 px-4 mt-2 bg-gray-200 border border-blue-600 rounded-md" readonly />
                  </div>
                </div>
              </div>
              <div>
              ...
              <div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="text" name="" id="br" value="<?php echo $reason; ?>" class="w-full py-2 px-4 mt-2 bg-gray-200 border border-blue-600 rounded-md" readonly />
                  </div>
                  <div>
              ...
              <div>
                ...
                <?php
                  if($slip == "No Slip Uploaded"){
                    echo '<div class="mt-2 rounded-md w-full bg-blue-200 text-blue-600 font-bold p-4"><p>No Slip Uploaded for this Payment</p></div>';
                  }elseif($slip == "Checked and Removed"){
                    echo '<div class="mt-2 rounded-md w-full bg-blue-200 text-blue-600 font-bold p-4"><p>Uploaded Slip is Checked and Remove</p></div>';
                  }elseif($slip == NULL){
                    echo '<div class="mt-2 rounded-md w-full bg-blue-200 text-blue-600 font-bold p-4"><p>No Slip Uploaded for this Payment</p></div>';
                  }else{
