      ...
      <div class="drawer-content flex flex-col">
        ...
        <div class="md:grid md:grid-cols-2 md:grid-rows-1 gap-4 items-center">
            ...
            <form action="" method="post">
              ...
              <div class="relative">
                ...
                <input
                  ...
                  placeholder="Search renewal info"
          ...
          <div class="absolute right-0 pr-4">
            <button
              onclick="window.location.href='add_renew.php'" 
              type="button"
            ...
            >
            <i class="fa-solid fa-retweet mr-4"></i> Add Renewal
            </button>
          </div>
        </div>
        ...
        <?php
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $query = "SELECT * FROM renew WHERE reId LIKE :find OR fname LIKE :find OR branch LIKE :find OR nic LIKE :find OR barcode LIKE :find;";
                ...
                $query = "SELECT * FROM renew ORDER BY reId DESC;";
                $rows = $conn->prepare($query);
                $rows->execute();
                $result = $rows->fetchAll(PDO::FETCH_ASSOC);
            ...
            if ($result) {
                foreach ($result as $row) {
                    ...
                    echo '
                        <div class="bg-blue-200 rounded-xl shadow-md overflow-hidden" onclick="window.location.href=\'renew_details.php?rid='.$reId.'&cGFpZCBhdA\'">
                        <div class="p-4 flex items-center">
                          <div class="bg-gray-600 w-16 h-16 p-4 rounded-full text-center">
                            <p class="text-xl font-bold text-white">'.$reId.'</p>
                          </div>
                          <div class="ml-4">
                            <div class="flex items-center space-x-2 text-sm">
                              <div class="text-gray-500">' . $opens . '<span class="bg-yellow-200 font-semibold rounded-md ml-2 px-4 py-1">'.$branch.'</span></div>
                            </div>
                            <p class="mt-2 text-gray-600">BarCode : '.$barcode.'</p>
                            <p class="mt-2 font-bold text-gray-600">Exp. Date : '.$exp.'</p>
                          </div>
                        </div>
                      </div>
                    ';
