        ...
        <?php
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
              ...
              if(isset($_POST['search'])) {
                  ...
                  $query = "SELECT s.*, u.fname AS fname, u.lname AS lname 
                            FROM student s 
                            LEFT JOIN users u ON s.userId = u.userId 
                            WHERE s.userId LIKE :find 
                               OR s.stuId LIKE :find 
                               OR s.addr LIKE :find 
                               OR s.fullName LIKE :find 
                               OR s.nic LIKE :find 
                               OR s.tel LIKE :find 
                               OR s.passNo LIKE :find 
                               OR s.email LIKE :find";
                  ...
                  if($rows->execute()) {
                      ...
                      print_r($rows->errorInfo()); // Output PDO error info
                  }
              }
              
            } else {
                // If search form is not submitted, fetch all records
                $query = "SELECT s.*, u.fname AS fname, u.lname AS lname FROM student s LEFT JOIN users u ON s.userId = u.userId ORDER BY s.userId DESC;";
                $rows = $conn->prepare($query);
                $rows->execute();
                $result = $rows->fetchAll(PDO::FETCH_ASSOC);
                ...
                foreach ($result as $row) {
                    ...
                    >
                      ...
                      <div class="p-4 font-semibold items-start">
                        <p class="text-xs text-blue-600">RPSTU-'.$ids.'</p>
                        <p class="text-lg" >'.$name.'</p>
                        <div class="join">
                          <button
                            class="btn btn-xs join-item bg-blue-700 hover:bg-blue-800 border-none text-white"
                          type="button" onclick="window.location.href=\'profile.php?uid='.$ids.'&vxc='.$name.'&zxb='.$lname.'\'">
                            <i class="fas fa-solid fa-eye"></i>View
                          </button>
                          <button
                            class="btn btn-xs bg-red-600 hover:bg-red-700 text-white border-none join-item delete-btn" data-user-id="'.$ids.'"
