...
const MedicalTable = ({ Medicals}) => {
	//fuction given by tanstack table to create columns in table - Commented by CYCO
	const columnHelper = createColumnHelper();

   const [medical, setMedical] = useState(Medicals);

	// Handle Delete function
	const handleDelete = (del_id) => {
		Swal.fire({
			title: "Are you sure?",
			text: "Do you want to delete this Medical Request? This action cannot be undone.",
			icon: "warning",
			...
			if (result.isConfirmed) {
				axios
					.delete(`http://localhost:4000/api/medi/del/${del_id}`)
					.then(() => {
						setMedical(medical.filter((medical) => medical.id !== del_id));
						Swal.fire(
							"Deleted!",
							"The Medical Request has been deleted.",
							"success",
	...
	const columns = [
		...
		columnHelper.accessor("req_date", {
			cell: (info) => info.getValue(),
			header: () => <span className='flex items-center'>Req. Date</span>,
		}),
		...
		columnHelper.display({
			...
			cell: ({ row }) => (
				<div className='flex space-x-2 justify-start'>
					<Link
						to={`/medi/details/${row.original.id}`}
						className='text-blue-500 hover:text-blue-700'
						aria-label='View'>
						<Eye className='w-5 h-5' />
					</Link>
					<Link
						to={`/medi/${row.original.id}?role=branch`}
	...
	if (!Medicals || Medicals.length === 0) {
		return <div>No Medicals available</div>;
	}
	...
	const table = useReactTable({
		data: Medicals, //payment is the data set pass from payment page - Commented by CYCO
		columns,
