...
const ExamStuTable = ({ studentData, ScheduleId }) => {
  ...
  const scheduleId = ScheduleId
    ...
    const handleUpdate = async (stu_id) => {
			...
			// Show SweetAlert2 with a select dropdown for result
			const { value: newResult } = await Swal.fire({
				title: "Update Student Result",
				html: `
      <select id="resultSelect" class="swal2-select">
    ...
    `,
				...
				preConfirm: () => {
					const selectedValue = document.getElementById("resultSelect").value;
			...
			if (newResult !== null) {
				try {
					const response = await axios.put(
						`http://localhost:4000/api/exam/result/${scheduleId}/${stu_id}`,
						{
							result: newResult,
						},
					);
					...
					if (response.data?.success) {
						Swal.fire({
							...
							text: `Student's result has been updated to ${newResult}`,
						...
						Swal.fire({
							icon: "error",
							title: "Update Failed",
							text: response.data.message || "Failed to update result.",
						}).then(() => {
							window.location.reload();
						});
					}
					...
					Swal.fire({
						icon: "error",
						title: "Error",
						text: "An error occurred while updating the result.",
					}).then(() => {
						window.location.reload();
					});
				}
  ...
  const columns = [
		...
		columnHelper.accessor("result", {
			cell: (info) => info.getValue(),
			header: () => <span className='flex items-center'>Result</span>,
		}),
