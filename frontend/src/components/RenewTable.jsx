...
const RenewTable = ({ Renews }) => {
	//fuction given by tanstack table to create columns in table - Commented by CYCO
	const columnHelper = createColumnHelper();

    const [renewals, setRenewals] = useState(Renews);

	// Handle Delete function
	const handleDelete = (del_id) => {
		Swal.fire({
			title: "Are you sure?",
			text: "Do you want to delete this Renew Request? This action cannot be undone.",
			icon: "warning",
			...
			if (result.isConfirmed) {
				axios
					.delete(`http://localhost:4000/api/renewal/del/${del_id}`)
					.then(() => {
						setRenewals(renewals.filter((renewal) => renewal.id !== del_id));
						Swal.fire(
							"Deleted!",
							"The renewal Request has been deleted.",
							"success",
					...
					.catch(() =>
						Swal.fire(
							"Error",
							"Failed to delete the renewal Request!",
							"error",
						).then(() => window.location.reload()),
					);
			}
	...
	const columns = [
		...
		columnHelper.accessor("reId", {
			cell: (info) => info.getValue(),
			header: () => <span className='flex items-center'>Renewal ID</span>,
		}),
		columnHelper.accessor("name", {
			cell: (info) => info.getValue(),
			header: () => <span className='flex items-center'>Request by</span>,
		}),
		columnHelper.accessor("reType", {
			cell: (info) => info.getValue(),
	...
	if (!Renews || Renews.length === 0) {
		return <div>No Renews available</div>;
	}
	...
	const table = useReactTable({
		data: Renews, //payment is the data set pass from payment page - Commented by CYCO
		columns,
